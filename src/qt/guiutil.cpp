...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("pipshares"))
        return false;
...
{
    // Convert pipshares:// to pipshares:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pipshares://"))
    {
        uri.replace(0, 12, "pipshares:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "pipshares.lnk";
}
...
{
    return GetAutostartDir() / "pipshares.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Pipshares-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  pipshares-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("pipshares-Qt"));
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QPushButton    { background: rgb(222,222,222); color: rgb(21,21,21); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(222,222,222); border-left-style: solid; border-left-width: 6px; }"
